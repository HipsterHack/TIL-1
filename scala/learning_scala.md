...
println(friend)
```
* 패턴 매칭으로 사용하는 경우엔 case class를 이용한다. 
* 최상위 클래스는 sealed 처리를 한다. 이러면 다른 파일에서 클래스 생성이 불가능하다.
* 그리고 모든 클래스의 경우가 만족이 안될때 컴파일러가 경고를 해준다
* List, Tree 도 모두 같은 형태의 자료구조를 이용한다.
