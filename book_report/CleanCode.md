...
변수를 비공개하는 이유는 다른 객체가 변수에 의존하지 않게 만들려는 것이다.

## 자료 추상화

자료를 세세하게 공가하는것 보단 추상개념을 사용하는게 낫다.
아무 생각없이 조회 설정 함수를 추가하는 방법이 제일 나쁘다.

## 데이터 객체 비대칭 

절차 코드는 기존 자료구조를 내비두면서 새 함수를 추가하기 쉽다.
반면 객체지향 코드는 기존함수를 변경하지 않으면서 새 클래스 추가하기 쉽다.

절차코드는 새 자료구조를 추가하기 어렵다.
